...
class Seq2seq():
    ...
    def __init__(self):
        ...
        self.vec_to_char = {v:k for k,v in list(self.decoder_vocab.items())}
    ...
    if sys.argv[1]:
        ...
        if sys.argv[1] == 'train':
            ...
            while aa<100:
                ...
                myanswer = seq.predict(myquestion)
                print("回答: ", myanswer[:-7])
                # myanswer.replace("__EOS__",'。',3)
                # if myanswer.find("__EOS__"):
                #      content1,content2 = myanswer.split('__EOS__')
                #      print("回答: ", content1)
                # else:
                #     print("回答: ", myanswer)
                aa += 1;